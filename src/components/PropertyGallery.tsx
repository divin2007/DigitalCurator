      ...
      <div className="grid grid-cols-12 gap-8 items-start">
        ...
        <div className="col-span-12 md:col-span-5 lg:col-span-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          {images.slice(0, 2).map((img, i) => (
            <div key={i} className="overflow-hidden rounded-sm h-64 md:h-[242px]">
              <img className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" src={img} alt={`Property view ${i + 1}`} />
            </div>
          ))}
          {images.length > 2 && (
             <div className="overflow-hidden rounded-sm relative h-64 md:h-[242px] md:col-span-2">
                <img className="w-full h-full object-cover" src={images[2]} alt="Property gallery preview" />
