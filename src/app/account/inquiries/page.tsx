  ...
  return (
    <div className="space-y-16">
      ...
      <div className="space-y-6">
        {inquiries.map((inq, i) => (
          <div key={i} className={`flex flex-col md:flex-row md:items-center justify-between p-8 border ${inq.status === 'Action Required' ? 'bg-surface-container-low border-primary/20' : 'bg-surface-container-lowest border-outline-variant/20'} editorial-shadow group gap-8`}>
            <div className="flex items-center gap-8">
              <div className={`w-24 h-24 overflow-hidden relative shrink-0 ${!inq.property && 'bg-primary/10 flex items-center justify-center'}`}>
                {inq.property ? (
